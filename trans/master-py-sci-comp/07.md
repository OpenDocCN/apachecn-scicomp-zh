...
熊猫输入输出API是一组返回熊猫对象的读取器函数。使用熊猫捆绑的工具加载数据非常容易。数据从各种类型文件中的记录加载到 pandas 数据结构中，如 **逗号分隔值** ( **CSV** )、Excel、HDF、SQL、JSON、HTML、谷歌大查询、pickle、stats 格式和剪贴板。有几个阅读器功能—每种文件一个功能—即`read_csv`、`read_excel`、`read_hdf`、`read_sql`、`read_json`、`read_html`、`read_stata`、`read_clipboard`和`read_pickle`。加载后，数据准备好进行分析。这包括删除错误条目、规范化、分组、转换和排序。
