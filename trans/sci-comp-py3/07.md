...
![Basics](images/B05511_07_01-1.jpg)。
...
![Variable number of arguments](images/starred_arguments_on_call.jpg)
...
![Variable number of arguments](images/starred_arguments_at_definition.jpg)
...
例如，下面的函数取一个复数 *z* ，并根据欧拉公式将其极坐标表示返回为幅度 *r* 和角度![Return values](images/varphi.jpg):

![Return values](images/euler.jpg)
...
![Recursive functions](images/chebychev.jpg)
...
函数![Partial application](images/fullsine.jpg)可以看作是两个变量的函数。人们通常认为ω不是自由变量，而是函数族的固定参数:

![Partial application](images/partialsine.jpg)
...
名称*λ*源于微积分和数理逻辑的一个特殊分支，![Anonymous functions - the  lambda keyword](images/lambda.jpg)-微积分。

例如，为了计算下面的表达式，我们可以使用 SciPy 的函数`quad`，它要求函数被积分作为它的第一个参数，积分界限作为接下来的两个参数:

![Anonymous functions - the  lambda keyword](images/B05511_07_02.jpg)
...
**例 1** →编写一个函数`polar_to_comp`，该函数接受两个参数 *r* 和![Exercises](images/varphi.jpg)并返回复数![Exercises](images/euler.jpg)指数函数使用 NumPy 函数`exp`。
...
![Exercises](images/subintervals.jpg)

如果符号在第一个子区间 *b* 中发生变化，则重新定义为:

![Exercises](images/B05511_07_05.jpg)

否则，它会以相同的方式重新定义为:

![Exercises](images/B05511_07_06.jpg)
...
![Exercises](images/B05511_07_07.jpg)

写一个计算两个整数的最大公约数的函数。编写另一个函数，使用以下关系计算这些数字的最小公倍数:

![Exercises](images/B05511_07_08.jpg)
